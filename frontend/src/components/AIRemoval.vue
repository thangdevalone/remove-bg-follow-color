...
const processImage = async () => {
  ...
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/remove-background`, {
      method: 'POST',
      body: formData
    })
...
const downloadResult = () => {
  ...
  link.download = `removed-bg.${options.format.toLowerCase()}`
  link.click()
}
...
<template>
  <div class="bg-white rounded-xl shadow-lg p-6">
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-gray-800 mb-2">AI Background Removal</h2>
      <p class="text-gray-600">
        Use advanced AI to automatically detect and remove backgrounds from your images
      </p>
    </div>
    ...
    <div v-if="resultImage" class="space-y-4">
      ...
      <div class="grid md:grid-cols-2 gap-4">
        ...
        <!-- Result -->
        <div>
          <h4 class="text-sm font-medium text-gray-600 mb-2">Background Removed</h4>
